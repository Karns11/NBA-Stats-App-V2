  ...
  return (
    <div>
      ...
      {todaysOdds.map((match, index) => {
        ...
        const awayTeamOutcome = match.bookmakers[0].markets[0].outcomes.find(
          outcome => outcome.name === match.away_team
        );

        const homeTeamOutcome = match.bookmakers[0].markets[0].outcomes.find(
        ...
        const oddsText =
          awayTeamOutcome && homeTeamOutcome
            ? <><span className='text-dark'>{match.away_team}:</span> <span className='text-success'>[{displayAwayTeamOdds}]</span> <span className='text-dark'>@</span> <br /><span className='text-dark'>{match.home_team}:</span> <span className='text-success'>[{displayHomeTeamOdds}]</span></>
            : 'Outcome not found';
        ...
        return (
          <div id='Todays-Games' className="col-md-4 mb-3" key={index}>
             <a className="game-link" rel="noreferrer" target='_blank' href={`https://sportsbook.draftkings.com/leagues/basketball/nba`}><div className="pe-5 text-center border border-dark p-3">
              <div className="game box">
